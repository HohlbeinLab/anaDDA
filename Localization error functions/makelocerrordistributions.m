function [locerrorpdf,locerrorpdfcorrected,rangex,locdist,rangefit] = makelocerrordistributions(rangeD,locerror,input)
...
for i = input.framerange
    ...
    if i > 1  % For i = 1 steps are uncorrelated so no correction required, for i>1 lauricella series are required for calculation of distribution
        while pass == 0
        range = rangeD(1)/i:binsize:min(max(rangex(:,i))*(locerror/i),maxcalculatedD); 
        ...
        conversion = (rangex(2,i)-rangex(1,i))/(binsize/(locerror/i));
        
        corrected=interp1(rangex(:,i),locdist(:,i),rangefit/(locerror/i))/conversion;
