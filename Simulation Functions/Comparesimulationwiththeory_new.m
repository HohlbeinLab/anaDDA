...
for j = 1:length(input.frametimerange)
    ...
    for i = input.framerange
        ...
        [Dlistdatatemp] = SimulationLocalizationandConfinement_new(input,false);
...
for i = 1:numel(input.frametimerange)
    for j = input.framerange
        ...
        if numel(input.framerange) == 8
            ...
            hold on
            plotlog_new(framenr,parameters,D(:,D(3,:)==input.frametime), input,rangeD,bootstrapparamstd,1,false)
         else
            figure
            title(['D distribution for track length ' num2str(j) ' steps'])
            hold on
            plotlog_new(framenr,parameters,D(:,D(3,:)==input.frametime), input,rangeD,bootstrapparamstd,1,true)
        end
        end
