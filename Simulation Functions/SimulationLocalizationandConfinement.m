...
Dim_A2 = sqrt(2*Dim_A2*Steptime);
Dim_B1 = sqrt(2*Dim_B1*Steptime);
Dim_B2 = sqrt(2*Dim_B2*Steptime);
...
if confinement == 1
    ...
    sumcont = sum(cont);
     ...
     startpos(cont,1:3)= fovsize*rand(sumcont, 3,'single');
end
...
%     startpos = newpos;
    t = t+Steptime;
    if  t > (runtime-0.5*Frametime) && (mod(t-runtime,Frametime) < 0.00000002 || mod(t-runtime,Frametime) > Frametime-0.00000002) 
...
if densitylocalizations>0
    lengtharray = length(Poslist(:,4));
    numrepeats = ceil(lengtharray/densitylocalizations) + NFrames+1- mod(ceil(lengtharray/densitylocalizations),NFrames+1);
    k = Poslist(1:min(numrepeats,lengtharray),4);
    Poslist(:,4) = k(mod(0:lengtharray-1,length(k))+1);
    fulltrack = 1;
    trueparticles = sort(repmat(1:Nparticles,[1 NFrames+1]));
    for i = 1:max(trueparticles)
        Poslist(trueparticles==i,4)=Poslist(trueparticles==i,4)+round((NFrames+1)*rand());
    end
end
...
if fulltrack == 1 % the full tracking script
    ...
    MSDav = zeros(nMolecules,1);
    for ii = 1:nMolecules
    xx = find(tracks(:,4)==ii);
    numberofframes(ii) = numel(xx)-1; 
% sum all squared displacement in the track
    if numel(xx)>1
        for jj = 1:numel(xx)-1               
            MSDav(ii) = MSDav(ii) + ((tracks(xx(jj+1),1) - tracks(xx(jj),1))^2 +...
                        (tracks(xx(jj+1),2) - tracks(xx(jj),2))^2);
        end
            MSDav(ii) = MSDav(ii)/jj; % mean square displacement
    else
        MSDav(ii)=NaN;
    end
    end
    ...
    %Dfit = Dfit(Dfit>0);
    Dfit = Dfit';
    Dfit(2,:) = numberofframes;
    Dfit(3,:) = input.frametime; 
%     percentageerror = [tracks(:,4) trueparticles'];
%     percentageerror(~isnan(Dfit(1,:))',:);
    Dfit = Dfit(:,~isnan(Dfit(1,:))); 
%     for i = 1:max(percentageerror(:,1))
%     perc(i) = numel(unique(percentageerror(percentageerror(:,1)==i,2)));
%     end
%     sum(perc>1)/numel(perc)
else 
